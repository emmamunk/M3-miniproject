...
```{r}
index1 <- createDataPartition(y = data$price_intervals_nlp, p = 0.8, list = FALSE)
...
tokenizer_train$document_count
...
trained_model <- model %>% fit(
    x = train_pad, # sequence we're using for prediction 
    y = y, # sequence we're predicting
    batch_size = 256, # how many samples to pass to our model at a time
    epochs = 10, # how many times we'll look @ the whole dataset
    validation_split = 0.25)
mean(trained_model$metrics$val_accuracy)
...
```{r}
x_train_nlp = pad_sequences(tweet_tokenizer, maxlen = 250)
x_test_nlp = pad_sequences(tweet_tokenizer2, maxlen = 250)
```
...
text_out = main_input %>% 
  layer_embedding(input_dim = 10000, output_dim = 512, input_length = 250) %>% 
  layer_lstm(units = 32)
