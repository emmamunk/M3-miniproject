...
Our dependent variables is Airbnb price. 
...
In this project, we want to look at catogorical variables. Here we set the max price to 5500 dollars and split by 500, so we'll get 11 intervals. We had some outliers with higher prices, but they only had 1 or 2 observations, so we decided to remove them.
...
And now we have to change the names, as the models will not run to names, unfortunately.
...
There is all in all four different types of rooms, but shared rooms and hotel rooms were very small combined, so we decided only to look at entire home/apartments or private room and define the variable as a binary variable. Here entire home/apartment is equal to 1 and private room is equal to 0. 
...
Here, it's especially interesting to see what variables price is correlated with. Here, especially accommodates and bedrooms are highly positive correlated and zipcode and number of reviews are slightly negative correlated. And now we can run how the listings look like. Here there's 7776 observatiosna and 10 variables.
...
Here, we want to look at the listing and we're choosing not to look at price and id. 

```{r}
listings_ml = listings %>% select(c(-price,-id, -price_intervals_nlp))
```
...
```{r}
fit.nnet = train(price_intervals ~ ., training, 
              method='nnet', 
              trace = FALSE)
```
...
model = keras_model_sequential() %>%
  layer_dense(units = 16, activation = "relu", input_shape = c("5686")) %>%
...
bi_input = layer_input(shape = c(5))

# Text model
main_input = layer_input(shape = c(250))
...
```{r}
x_train <- pad_sequences(tweet_tokenizer, maxlen = 100)
...
trained_model <- model %>% fit(
    ...
    batch_size = 2048, # how many samples to pass to our model at a time
