...
```{r}
training_pad <- pad_sequences(text_seqs_training, maxlen = 250)
test_pad <- pad_sequences(text_seqs_test, maxlen = 250)
...
model <- keras_model_sequential() %>%
  layer_embedding(input_dim = 10000, output_dim = 250) %>%
  layer_simple_rnn(units = 250) %>% 
...
trained_model <- model %>% fit(
    x = training_pad, # sequence we're using for prediction 
    y = test_pad, # sequence we're predicting
    batch_size = 32, # how many samples to pass to our model at a time
